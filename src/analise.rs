...
pub fn get_sma_vec(data: (&Vec<f64>, &Vec<f64>, &Vec<f64>, &Vec<f64>), base: usize) -> (Vec<f64>, Vec<f64>) {
    ...
    for i in base..t.len() {
        let cur_sma_x: f64 = x[i - base..i].iter().sum::<f64>() / base as f64;
        let cur_sma_y: f64 = y[i - base..i].iter().sum::<f64>() / base as f64;
        let cur_sma_z: f64 = z[i - base..i].iter().sum::<f64>() / base as f64;
...
pub fn gnu_plot_test(data: &Vec<(f64, f64, f64)>) {
    ...
    let (sma_t3, sma_v3) = get_sma_vec((&t, &x, &y, &z), 150);
